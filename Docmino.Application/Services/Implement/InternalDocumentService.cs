...
{
    private readonly IValidator<InternalIncomingDocumentRequest> _incomingDocumentValidator;
    private readonly IValidator<InternalOutgoingDocumentRequest> _outgoingDocumentValidator;
    ...
    public InternalDocumentService(IUnitOfWork unitOfWork,
                                   ...
                                   IValidator<InternalIncomingDocumentRequest> incomingDocumentValidator,
                                   ...
                                   IValidator<InternalOutgoingDocumentRequest> outgoingDocumentValidator)
    {
    ...
    public async Task<Result> GetAll(InternalDocumentFilterRequest filter)
    {
        Expression<Func<Document, bool>> queryFilter = x =>
                                                ...
                                                && (filter.Departments.IsEmpty() || (x.DepartmentId != null && filter.Departments!.Contains(x.DepartmentId.Value)))
        ...
        var res = await _documentRepository.GetByFilterAsync(filter.PageSize, filter.PageNumber,
                                                     ...
                                                     selectQuery: InternalDocumentMapping.SelectResponseExpression);
        return FilterResult<List<InternalDocumentResponse>>.Success(res.Data.ToList(), res.TotalCount);
    }

    public async Task<Result> GetById(Guid id)
    {
        var selectedEntity = await _documentRepository
                                    .FirstOrDefaultAsync(x => x.Id == id, selectQuery: InternalDocumentMapping.SelectDetailResponseExpression);
        ...
        return Result<InternalDocumentDetailResponse>.SuccessWithBody(selectedEntity);
    }

    public async Task<Result> AddIncomingDocument(InternalIncomingDocumentRequest modelRequest, List<IFormFile>? files)
    {
        //Validate
    ...
    public async Task<Result> UpdateIncomingDocument(Guid id, InternalIncomingDocumentRequest modelRequest, List<IFormFile>? files)
    {
        //Validate
        var validateResult = _incomingDocumentValidator.Validate(modelRequest);
        if (!validateResult.IsValid)
    ...
    public async Task<Result> AddOutgoingDocument(InternalOutgoingDocumentRequest modelRequest, List<IFormFile>? files)
    {
        //Validate
    ...
    public async Task<Result> UpdateOutgoingDocument(Guid id, InternalOutgoingDocumentRequest modelRequest, List<IFormFile>? files)
    {
        //Validate
        var validateResult = _outgoingDocumentValidator.Validate(modelRequest);
        if (!validateResult.IsValid)
    ...
    private async Task<Checker> CheckAddIncomingDocument(InternalIncomingDocumentRequest documentRequest)
    {
        var checkAdd = await CheckInternalDocument(documentRequest);
        if (checkAdd.IsError())
        {
            return checkAdd;
        }
    ...
    private async Task<Checker> CheckEditIncomingDocument(Guid id, InternalIncomingDocumentRequest documentRequest)
    {
        var selectedDocument = await _documentRepository.FirstOrDefaultAsync(x => x.Id == id);
        if (selectedDocument == null)
        {
        ...
        {
            return Checker.Error(ErrorMessage.ObjectExisted(documentRequest.ArrivalNumber, "Số ký hiệu vb"));
        }
        return Checker.Success();
    }

    private async Task<Checker> CheckAddOutgoingDocument(InternalOutgoingDocumentRequest documentRequest)
    {
        var checkAdd = await CheckInternalDocument(documentRequest);
        if (checkAdd.IsError())
        {
    ...
    private async Task<Checker> CheckEditOutgoingDocument(Guid id, InternalOutgoingDocumentRequest documentRequest)
    {
        var selectedDocument = await _documentRepository.FirstOrDefaultAsync(x => x.Id == id);
        if (selectedDocument == null)
        {
    ...
    private async Task<Checker> CheckInternalDocument(InternalDocumentRequest documentRequest)
    {
        ...
        if (!await _departmentRepository.AnyAsync(x => x.Id == documentRequest.DepartmentId))
        {
            return Checker.Error(ErrorMessage.ObjectNotFound(documentRequest.DepartmentId, "Phòng ban"));
        }
        return Checker.Success();
    }
}
