...
public class InternalIncomingDocumentValidator : AbstractValidator<InternalIncomingDocumentRequest>
{
    public InternalIncomingDocumentValidator()
    {
        RuleFor(x => x.Name)
...
public class InternalOutgoingDocumentValidator : AbstractValidator<InternalOutgoingDocumentRequest>
{
    public InternalOutgoingDocumentValidator()
    {
        RuleFor(x => x.Name)
            .NotEmpty().WithMessage("Bạn cần phải nhập tên.")
            .MaximumLength(200).WithMessage("Tên không được vượt quá 200 ký tự.");
