...
[RoleAuthorize]
public class ExternalDocumentController : ApiControllerBase
{
    private readonly IExternalDocumentService _service;

    public ExternalDocumentController(IExternalDocumentService service)
    {
        _service = service;
    }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] ExternalDocumentFilterRequest filter)
    {
    ...
    {
        var parsedRequest = JsonConvert.DeserializeObject<ExternalIncomingDocumentRequest>(documentRequest);
    ...
    {
        var parsedRequest = JsonConvert.DeserializeObject<ExternalOutgoingDocumentRequest>(documentRequest);
        if (parsedRequest == null) return ApiResponse(Result.Error(HttpStatusCode.BadRequest, "Document must be not null!"));
